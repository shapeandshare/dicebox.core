...
logging.basicConfig(
    ...
    level=logging.DEBUG,
    filemode='w',
    filename="%s/trainning_processor.log" % config.LOGS_DIR
)


url = config.TRAINNING_PROCESSOR_SERVICE_RABBITMQ_URL
parameters = pika.URLParameters(url)
parameters.heartbeat = 0 # turn this off for now, the timeout otherwise
connection = pika.BlockingConnection(parameters=parameters)
channel = connection.channel()

channel.queue_declare(queue=config.TRAINNING_PROCESSOR_SERVICE_RABBITMQ_TRAIN_REQUEST_TASK_QUEUE, durable=True)
...
def train_call(trainning_request_id):
    logging.debug("processing trainning request id: (%s)" % trainning_request_id)
    ...
    while i <= config.EPOCHS:
        ...
        #path = "%s/%s/" % (config.WEIGHTS_DIR, trainning_request_id)
        ...
        full_path = "%s/%s.hdf5" % (config.WEIGHTS_DIR, trainning_request_id)
        logging.info("saving model weights after epoch %i to file %s" % (i, full_path))
...
def callback(ch, method, properties, body):
    ...
    train_call(json.loads(body)['trainning_request_id'])
    logging.info(" [x] Done")

channel.basic_qos(prefetch_count=1)
channel.basic_consume(callback,
                      queue=config.TRAINNING_PROCESSOR_SERVICE_RABBITMQ_TRAIN_REQUEST_TASK_QUEUE)
