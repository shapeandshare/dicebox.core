...
class LayerFactory(ABC):
    ...
    @staticmethod
    def build_conv2d_layer(filters: int, kernel_size: Tuple[int, int], strides: Tuple[int, int], padding: Conv2DPadding, activation: ActivationFunction) -> Conv2DLayer:
        return Conv2DLayer(filters=filters, kernel_size=kernel_size, strides=strides, padding=padding, activation=activation)
