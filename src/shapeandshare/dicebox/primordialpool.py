...
class PrimordialPool:
    ...
    def export_population(self, population_id: str, generation: int, population: List[DiceboxNetwork]) -> None:
        ...
        with open(output_file, "w") as file:
            ...
            for individual in population:
                individual_full = {"accuracy": individual.get_accuracy(), "genome": individual.decompile()}
                population_genome.append(individual_full)
            output = {"population": population_genome, "average_accuracy": self.get_average_accuracy(population)}
    ...
    def generate(self, population_file: str = None) -> str:
        ...
        optimizer: EvolutionaryOptimizer = EvolutionaryOptimizer(
            config=self.config, retain=0.4, random_select=0.1, mutate_chance=0.2
        )
        ...
        if population_file is None:
            ...
            with open(population_file, "r") as file:
                ...
                networks: List[DiceboxNetwork] = optimizer.create_population(
                    size=population_size, population_definition=population_raw
                )
